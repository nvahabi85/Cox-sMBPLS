...
#####################################################################
  ...
  y.orig.test = Ytest[, 3, drop=F] # To be used in the final Cox model (Valid)
  Ytest = Ytest[, 1, drop=F]} #Re-weighted y, not the original y!
  
  nblo = length(Xtrain)
  ntrain = nrow(Ytrain) # nb observations
  
  p = lapply(1:nblo, function(x) ncol(Xtrain[[x]]))
  index.p = lapply(1:nblo, function(x) c(1:ncol(Xtrain[[x]])))
  names(index.p) = list_name # nb covariates per block
  
  q = 1 # nb of responses (==1 for Cox reg)
  ...
  if (!is.null(Xtest)) {
  X.test = cbind.data.frame(sXtest)
  ncolX.test = ncol(X.test)
  is.na(X.test) = sapply(X.test, is.infinite)
  ...
  # Global loading/latent
  W = matrix(data=NA, nrow=ncolX, ncol=ncomp.max, dimnames = list(colnames(X), dimlab)) # spls weights; p is the total number of the features
  T = matrix(data=NA, nrow=ntrain, ncol=ncomp.max, dimnames = list(row.names(Ytrain), dimlab)) # spls latent components
  P = matrix(data=NA, nrow=ncomp.max, ncol=ncolX, dimnames = list(dimlab, colnames(X))) # regression of X over T
  Q = matrix(data=NA, nrow=ncomp.max, ncol=q, dimnames = list(dimlab, paste("Loading.y"))) # regression of Y over T
  ...
  Mnorm1 = list()
    ...
    what[[1]] = ust(M[[1]], lambda.l1)  # w-partial
    what[[2]] = ust(M[[2]], lambda.l1)
    what[[3]] = ust(M[[3]], lambda.l1)

    T_p[[1]] = X1[[1]] %*% what[[1]] #t-partial
    T_p[[2]] = X1[[2]] %*% what[[2]]
    T_p[[3]] = X1[[3]] %*% what[[3]]
...
# Y_hat
hatY = numeric(ntrain)
hatY = as.matrix(X) %*% betahat.bind
colnames(hatY) = c("y_hat")

# Residuals
residuals = sYtrain - hatY
colnames(residuals) = c("residual")
...
# Prediction.train
pred = predict(Cox.mbspls, data=data.final.cox, type="risk")  
cindex.train2 = concordance.index(x=pred, surv.time=data.final.cox$y, surv.event=data.final.cox$status, method="noether")$c.index


## C-index Test
## Test data preparation
ww = W
bb2 = cov2 # Block weights
# w
...
# Final test data-set
data.final.cox.test = cbind.data.frame(Ytest, status.test, as.data.frame(Ts))
# C-index Test
pred.test = predict(Cox.mbspls, data.final.cox.test)
cindex.test = concordance.index(x=pred.test, surv.time=data.final.cox.test$y, surv.event=data.final.cox.test$status, method="noether")$c.index
#####################################################################
#### Return Final Result
#####################################################################
result = list(Xtrain = Xtrain,
              ...
              Cindex.train2 = cindex.train2,
              
              pred.test = pred.test,
              cindex.test = cindex.test) 

class(result) = c("Multiblock", "sMBPLS", "Cox-sMBpls", "cox-PH")
return(result)
} # End of the function
